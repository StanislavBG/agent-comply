...
import type { ScanResult, RiskTier } from '../types/index.js';

// EU AI Act Annex III risk classification by detected context signals
// Methodology: scan file paths + matched patterns for high-risk use-case signals
...
const PROHIBITED_SIGNALS: RiskSignal[] = [
...
const LIMITED_SIGNALS: RiskSignal[] = [
...
function classifyFile(file: string, provider: string): { tier: RiskTier; reason: string } {
  const context = file.toLowerCase();

  for (const signal of PROHIBITED_SIGNALS) {
    if (signal.pattern.test(context)) return { tier: signal.tier, reason: signal.reason };
  }
...
const TIER_ORDER: Record<RiskTier, number> = { prohibited: 4, high: 3, limited: 2, minimal: 1 };
...
export function runClassify(targetPath: string): void {
  console.log(`\nScanning: ${targetPath}\n`);
  ...
  const byFileProvider = new Map<string, { tier: RiskTier; reason: string; provider: string; file: string; line: number }>();
  ...
  let projectTier: RiskTier = 'minimal';
  const entries = [...byFileProvider.values()];

  for (const e of entries) {
    if (TIER_ORDER[e.tier] > TIER_ORDER[projectTier]) projectTier = e.tier;
  }
  ...
  const tierCounts: Record<RiskTier, number> = { prohibited: 0, high: 0, limited: 0, minimal: 0 };
