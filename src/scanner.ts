...
export interface ScanResult {
  projectDir: string;
  aiImports: Array<{ file: string; line: number; import: string; provider: string }>;
  envKeys: Array<{ file: string; key: string; provider: string }>;
  promptTemplates: Array<{ file: string; line: number; snippet: string }>;
  modelReferences: Array<{ file: string; line: number; model: string; provider: string }>;
  configFiles: string[];
  summary: {
    totalFiles: number;
    filesWithAI: number;
...
const AI_IMPORT_PATTERNS: Array<{ pattern: RegExp; provider: string }> = [
...
const MODEL_STRING_PATTERNS: Array<{ pattern: RegExp; model: string; provider: string }> = [
...
const API_KEY_PATTERNS: Array<{ pattern: RegExp; provider: string }> = [
...
export function scanProject(projectDir: string): ScanResult {
  ...
  const envFiles = collectFiles(projectDir, ['.env']).concat(
    collectFiles(projectDir, ['']).filter(f => path.basename(f).startsWith('.env'))
  );
