...
interface Rules {
  risk_tiers: Record<string, { description: string; keywords: string[]; patterns: string[]; requirements?: string[] }>;
  model_patterns: Record<string, { models: string[]; default_tier: RiskTier }>;
  import_patterns: Array<{ pattern: string; provider: string }>;
}
...
function loadRules(): Rules {
  ...
  return yaml.load(content) as Rules;
}

function tierPriority(tier: RiskTier): number {
  const priorities: Record<RiskTier, number> = {
    unacceptable: 4,
    high: 3,
    limited: 2,
    minimal: 1,
...
export function classifyProject(projectDir: string): ClassificationResult {
  ...
  let overallTier: RiskTier = findings.length > 0 ? 'minimal' : 'unknown';
...
function analyzeFile(filePath: string, rules: Rules, projectDir: string): ModelFinding[] {
  const findings: ModelFinding[] = [];
  let content: string;
  try {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const importPattern of rules.import_patterns) {
      ...
      if (regex.test(line) && (line.includes('import') || line.includes('require') || line.includes('from'))) {
        ...
        const tier: RiskTier = providerConfig ? providerConfig.default_tier : 'limited';
...
function analyzeConfigFile(filePath: string, rules: Rules, projectDir: string): ModelFinding[] {
  const findings: ModelFinding[] = [];
  let content: string;
  try {
    content = fs.readFileSync(filePath, 'utf-8');
